...
function TimelineEntry({ entry, index }: { entry: typeof entries[0]; index: number }) {
  ...
  return (
    <div className="relative grid grid-cols-1 md:grid-cols-2 gap-20 md:gap-32 items-center mb-32 md:mb-48 last:mb-0">
      {/* ── Text Side ── */}
      <motion.div
      ...
      >
        <span className="font-playfair italic text-[#c5a47e] text-lg mb-4 tracking-widest">{year}</span>
        <p className="font-sans text-[11px] uppercase tracking-[0.3em] text-[#1a1a1a]/40 mb-3">{subtitle}</p>
        <h3 className="font-playfair text-[clamp(1.8rem,4vw,2.8rem)] font-light tracking-[0.1em] uppercase mb-8 text-[#1a1a1a] leading-tight">
          {title}
        </h3>
        <p className={`text-[15px] text-[#1a1a1a]/60 leading-[2] tracking-wide font-light max-w-[450px] ${imageLeft ? "" : "ml-auto"}`}>
          {body}
        </p>
      </motion.div>
      ...
      <div className="hidden md:block absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 z-20">
        <div className="w-4 h-4 rounded-full bg-[#c5a47e] border-4 border-white shadow-sm" />
      </div>
    </div>
  );
}
  ...
  return (
    <section ref={sectionRef} className="bg-white py-24 md:py-40">
      <div className="max-w-[1400px] mx-auto px-8 md:px-12 relative">
        ...
        >
          <p className="font-playfair italic text-[#c5a47e] text-lg mb-4 tracking-wide">
            Our Legacy
          </p>
          <div className="flex items-center justify-center gap-10">
            <span className="hidden md:block flex-1 h-px bg-[#1a1a1a]/10" />
            <h2 className="font-playfair text-[clamp(2rem,6vw,3.5rem)] font-light tracking-[0.3em] uppercase leading-tight whitespace-nowrap">
              New Readings
            </h2>
            <span className="hidden md:block flex-1 h-px bg-[#1a1a1a]/10" />
          </div>
